...
export function AdminDashboard() {
  ...
  const stats = [
    { label: "Total Users", value: adminUsers.length.toString(), icon: Users, color: "text-blue-600", bg: "bg-blue-50" },
    ...
    { label: "Pending KYC", value: kycUsers.filter((u) => u.status === "pending").length.toString(), icon: Shield, color: "text-amber-600", bg: "bg-amber-50" },
  ...
  const kycStatusBadge = (status: string) => {
    if (status === "pending") return <Badge variant="outline" className="text-xs bg-amber-50 text-amber-700 border-amber-200">Pending</Badge>;
                    ...
                    <TableBody>
                      {disputes.map((dispute) => (
                        <TableRow key={dispute.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline" className={`text-xs capitalize ${
                              dispute.status === "open" ? "bg-blue-50 text-blue-700 border-blue-200"
      ...
      <Dialog open={disputeDialogOpen} onOpenChange={setDisputeDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => void handleUpdateDispute()}>Update</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
