...
const DOC_OPTIONS = [
  { icon: FileText, label: "Passport", color: "bg-[#E8F0EF] text-[#004B49]" },
  { icon: File, label: "IELTS Result", color: "bg-green-50 text-green-500" },
  { icon: File, label: "Work Letter", color: "bg-[#FBF3E1] text-[#9c7a1f]" },
  ...
  if (!chatInfo) {
    return (
      <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
        ...
        <Link to="/messages">
          <button className="mt-3 text-xs text-[#004B49] font-semibold">Back to Messages</button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)]">
      ...
      <div className="bg-white px-4 py-3 flex items-center gap-3 border-b border-gray-100 flex-shrink-0">
        ...
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#004B49] to-[#00746f] flex items-center justify-center text-white font-black text-sm flex-shrink-0">
          {chatInfo.counterpartyInitial}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="font-bold text-gray-800 text-sm">{chatInfo.counterpartyName}</span>
            {chatInfo.verified && <CheckCircle size={13} className="text-[#D4AF37]" />}
          </div>
          <div className="text-xs text-gray-400">{chatInfo.counterpartyRole}</div>
        </div>
        <Link to="/transactions">
          <div className="bg-[#FBF3E1] border border-[#D4AF37]/30 rounded-xl px-2.5 py-1.5 text-center">
            <div className="text-[10px] font-bold text-[#9c7a1f]">🔒 ${chatInfo.orderAmount}</div>
            <div className="text-[9px] text-[#9c7a1f]/70">Escrow</div>
          </div>
        </Link>
      </div>

      {/* ESCROW BANNER — shown before the first message, for safety */}
      {showEscrowBanner && (
        <div className="mx-3 mt-2 bg-[#E8F0EF] border border-[#004B49]/15 rounded-xl px-3 py-2 flex items-center gap-2 flex-shrink-0">
          <Lock size={12} className="text-[#004B49] flex-shrink-0" />
          <span className="text-[11px] text-[#004B49] flex-1 font-semibold">
            🔒 {chatInfo.orderTitle} · Escrow Active
          </span>
          <button onClick={() => setShowEscrowBanner(false)}>
            <X size={13} className="text-[#004B49]/40" />
          </button>
        </div>
      )}

      {/* POLICY BANNER — shown before the first message, for safety */}
      {showPolicyBanner && (
      ...
      {/* MESSAGES */}
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-2 bg-[#F4F6F6]">
        {messages.length === 0 ? (
              ...
              return (
                <div key={msg.id} className={`flex flex-col gap-0.5 ${isMe ? "items-end" : "items-start"}`}>
                  <div className="bg-white border border-gray-100 rounded-2xl px-3 py-2.5 flex items-center gap-2.5 max-w-[75%] shadow-sm">
                    <div className="w-8 h-8 bg-[#E8F0EF] rounded-lg flex items-center justify-center flex-shrink-0">
                      <FileText size={16} className="text-[#004B49]" />
                    </div>
                    <div>
            ...
            return (
              <div key={msg.id} className={`flex flex-col gap-0.5 ${isMe ? "items-end" : "items-start"}`}>
                <div className={`px-4 py-2.5 rounded-2xl max-w-[78%] text-sm leading-relaxed whitespace-pre-line ${
                  isMe ? "bg-[#004B49] text-white rounded-br-sm" : "bg-white text-gray-800 rounded-bl-sm shadow-sm"
                }`}>
                  {msg.content}
                </div>
                <span className="text-[10px] text-gray-400 px-1">{time}</span>
      ...
      <div className="flex gap-2 overflow-x-auto px-4 py-2 flex-shrink-0 scrollbar-none bg-white border-t border-gray-50">
        {QUICK_REPLIES.map((q) => (
          <button key={q} onClick={() => setText(q)}
            className="flex-shrink-0 bg-gray-50 border border-gray-100 rounded-full px-3 py-1.5 text-xs font-medium text-gray-600 hover:border-[#004B49]/40 hover:text-[#004B49] transition-all">
            {q}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white border-t border-gray-100 px-3 py-2.5 flex items-center gap-2 flex-shrink-0">
        <button onClick={() => setShowAttach(!showAttach)}
          className={`p-2 rounded-xl transition-all ${showAttach ? "bg-[#004B49] text-white" : "bg-gray-50 text-gray-400"}`}>
          <Paperclip size={18} />
        </button>
        ...
        <button onClick={() => void send()} disabled={!text.trim()}
          className="w-10 h-10 rounded-xl bg-[#004B49] flex items-center justify-center flex-shrink-0 disabled:opacity-40">
      ...
      {showSuspendWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
          ...
          <div className="relative bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <Link to="/help">
              <button className="w-full bg-[#004B49] text-white font-bold py-3.5 rounded-2xl text-sm mb-2">Contact Support</button>
            </Link>
      ...
      {showDocConfirm && (
        <div className="fixed inset-0 z-50 flex items-end">
          ...
          <div className="relative w-full max-w-lg mx-auto bg-white rounded-t-3xl p-6 pb-10">
            <div className="text-center mb-4">
              <div className="w-12 h-12 bg-[#E8F0EF] rounded-full flex items-center justify-center mx-auto mb-3">
                <FileText size={22} className="text-[#004B49]" />
              </div>
            ...
            <div className="bg-[#E8F0EF] border border-[#004B49]/15 rounded-xl p-3 mb-4 flex gap-2">
              <Shield size={14} className="text-[#004B49] flex-shrink-0 mt-0.5" />
              <div className="text-[11px] text-[#004B49]">Your document stays within Crossing. The other party cannot share it externally.</div>
            </div>
            <button onClick={() => void confirmUpload()} className="w-full bg-[#004B49] text-white font-bold py-4 rounded-2xl text-sm mb-2">
              ✓ Confirm Share
            </button>
